...
class Scanner:
    ...
    def consume(self, *expected_tokens):
        ...
        token, value = self.current_token # Get current token and its value (if any)
        if token in expected_tokens:
            self.current_token = self.get_token() # Get new token
            if token in [Token.ID, Token.NUM]: # Is it a number or identifier?
                return token, value # Return that
            else:
                return token # Not a number or identifier
        else:
...
class Token:
    ...
    NUM   = 'NUM'
    ID    = 'ID'
    READ  = 'READ' # Declaring READ as a thing
    WRITE = 'WRITE' # Declating WRITE as a thing
    ...
    token_regexp = [
        ...
        (THEN,  'then'),
        (WHILE, 'while'),
        (READ,  'read'), # READ implementation
        (WRITE, 'write'), # WRITE implementation
        (SEM,   ';'),
        (BEC,   ':='),
        (LESS,  '<'),
        (EQ,    '='),
        ...
        (ID,    '[a-z]+'),
        (NEQ,   '!='), # NEQ Implementation
        (MUL,   '\\*'), # MUL Implementation
        (DIV,   '/'), # DIV Implementation
        (NUM,   '[0-9]+'), # NUM Implementation
    ]
...
class If_Else_AST:
    '''
    Introduce a new class If_Else_AST for if-then-else-end statements and implement the methods __init__, __repr__ and indented.
    The class will be similar to If_AST. The method If_Else_AST.indented should output the label If-Else followed by the condition, the then-part and the else-part.
    '''
...
def write():
    '''Extend the recursive-descent parser by the new statement write e
    that prints the value of the expression e.
    To this end, modify the method statement and introduce a new method write which constructs the syntax tree using Write_AST
    '''
...
def read():
    '''
    Extend the recursive-descent parser by the new statement read i
    that reads a value and stores it in the variable with identifier i.
    To this end, modify the method statement and introduce a new method read which constructs the syntax tree using Read_AST.
    '''
